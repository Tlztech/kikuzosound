...
// components
import EditModal from "../AsculaideLibraryEdit";
...
import IpaxUrlSwitch from "../../../presentational/molecules/IpaxUrlSwicth";
//images
      ...
      <>
        ...
        {isEditModalVisible && (
          <EditModal
            ...
            onHideEditModal={() => handleEditModalVisible(this, false, null)}
            editItem={JSON.parse(JSON.stringify(editItem))}
            updateAusculaideData={(data, index) =>
              this.props.updateAusculaideData(data, index)
            }
          />
        )}
        <DeleteModal
...
const onDragEnd = (result, table_data, context) => {
  ...
  context.props.updateAusculaideOrder(items, context.props.userInfo.id);
};
...
// Export
//===================================================
export default withTranslation("translation")(AsculaideLIbrary);
