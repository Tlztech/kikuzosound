<?php
  ...
  if ($route_name === 'member_login') {
      ...
      if ($login_status == 0) {
          ...
          $btnLoginClass = "buttonLogin";
      } else {
          $str_label = trans('app.logout');
          $btnLoginClass = "buttonLogout";
      ...
      //ボタン
      $strBtnLogin = '<button' . $display_none . ' type = "button" name = "btn_login" value = "btn_login" class = "button_head '.$btnLoginClass.'" onclick = "location.href=' . $str_href . '">' . $str_label . '</button>';
  }
  if ($Debug) {
...
@endif
            ...
            <div class="headlogo">
                ...
                <div class="lang_switcher">
                    ...
                    <select onchange="location = this.value;" >
                        @if (env('lang_en'))
                            <option value="{{ route('en') }}" 
                                <?php if (config('app.locale')=='en' || !config('app.locale')) {
                                    echo "selected";
                                } ?>><a>‎‎‏‏‎  ‎@lang('app.en')</a></option>
                        @endif
                        @if (env('lang_ja'))
                            <option value="{{ route('ja') }}" 
                                <?php if (config('app.locale')=='ja') {
                                    echo "selected";
                                } ?>><a>@lang('app.jp')</a></option>
                        @endif
                    </select>
                        ...
                        if ($loggedUserId) {
                            ...
                            if ($hasExamAccess == 1) {
                                ?>
                                    ...
                                    <li class="@if( $route_name=='exams'           ) active @endif"><a
                                        href="/member_logout">{{$str_label}}</a></li>
                                <?php
                            }
                        }
    ...
    <?php
            ...
            if (Session::has("MEMBER_3SP_ACCOUNT_ID")) {
                ?>
                    <script>
                        //Warning modal for orientation change
                        var contents = document.createElement("div");
                        contents.innerHTML ="<h2 class='h2-title'>@lang('app.warning')</h2><p class='landscape-warning'>@lang('app.warning_details')<br><br></p><input type='checkbox' id='checkboxWarningModal' name='checkbox' value='check'> @lang('app.checkbox_label')<br><br>";
                        const matchesMediaQuery = window.matchMedia('(max-width:823px)').matches;
                        if (matchesMediaQuery) {
                            if(localStorage.getItem("warning_modal") != 1 && localStorage.getItem("warning_modal_show") != 1) {
                                swal({
                                    content: contents,
                                }).then(
                                    function (isConfirm) {
                                        localStorage.setItem("warning_modal_show", 1);
                                        if (isConfirm) {
                                            if($("#checkboxWarningModal").is(":checked")) {
                                                localStorage.setItem("warning_modal", 1);
                                            }
                                        }
                                    }
                                );
                            ...
                            if(swal.getState().isOpen) { // check if swal is open
                                swal.close();
                            }
                        }
                ...
                echo '<script>localStorage.setItem("warning_modal_show", 0)</script>';
            }
