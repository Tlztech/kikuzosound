...
<div class="well library-file">
  ...
  <audio src="{{ $stetho_sound->sound_path.'?_v='.session('version') }}" controls preload></audio>
  @else
  聴診音ファイルが指定されていません。
  @endif
  <br/><br/><strong>動画ファイル：</strong><br/><br/>
  @if($stetho_sound->video_path)
  <div class="row stetho_sound_images">
    <div style="" class="col-md-3 text-center">
      <video src="{{ $stetho_sound->video_path.'?v='.session('version') }}" controls style="width:300px; height:200px;"></video>
    </div>
  </div>
  @else
  ビデオファイルが指定されていません。
