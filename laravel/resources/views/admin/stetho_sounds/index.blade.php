...
<div class="page-header clearfix">
  <h1 class="pull-left">聴診音ライブラリ一覧</h1>
</div>
  ...
  <div class="col-md-12">
    <table class="table table-striped">
      <thead>
        <tr>
          ...
          <th>更新日時</th>
          @if(Auth::user()->role == App\User::$ROLE_ADMIN)
          <th><a class="btn btn-primary pull-right" href="{{ route('admin.stetho_sounds.create') }}">追加</a></th>
          @endif
        </tr>
      </thead>
      ...
      <tbody id="<?php echo isset($_GET['reorder']) ? "stetho_sound_list__tbody" : "" ?>">
        ...
        <tr>
          ...
          <td class="text-break"><a href="{{ route('admin.stetho_sounds.show', $stetho_sound->id) }}">{{$stetho_sound->title_en}}</a></td>
          <?php $type_strings = [ 1 => '肺音', 2 => '心音', 3 => '腸音', 9 => 'その他' ]; ?>
          <td>{{$type_strings[$stetho_sound->type] or '指定なし'}}</td>
          <td class="text-break">{{$stetho_sound->area_en}}</td>
          ...
          @if(Auth::user()->role == App\User::$ROLE_ADMIN)
          <td class="text-right">
            <a class="btn btn-success " href="{{ url('admin/stetho_sounds/'.$stetho_sound->id.'/edit?page='.$stetho_sounds->currentPage()) }}">編集</a>
            <form action="{{ route('admin.stetho_sounds.destroy', $stetho_sound->id) }}" method="POST" style="display: inline;" onsubmit="if(confirm('本当に削除しますか？この操作は取り消しできません。')) { return true } else {return false };">
    ...
    @if(Auth::user()->role == App\User::$ROLE_ADMIN)
      <div class="col-sm-offset-10 col-sm-2">
        <a class="btn btn-primary pull-right" href="{{ route('admin.stetho_sounds.create') }}">追加</a>
      </div>
    @endif

    <div class="col-sm-offset-4 col-sm-8 sorting-button">
      <a class="btn btn-primary" href="/admin/stetho_sounds<?php echo isset($_GET['reorder']) ? "" : "?reorder" ?>">
...
$(document).ready(function(){
  $("#stetho_sound_list__tbody").sortable({
    update: function(event, ui) {
      ...
      var data = JSON.stringify({"stetho_sounds": arr});
      $.ajax({
