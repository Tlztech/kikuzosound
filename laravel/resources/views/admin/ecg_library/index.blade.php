...
<div class="page-header clearfix">
    <h1 class="pull-left">心電図ライブラリ一覧</h1>
</div>
...
<div class="row">
    <div class="col-md-12">
        <table class="table table-striped">
            <thead>
                <tr>
                    ...
                    <th>更新日時</th>
                    @if(Auth::user()->role == App\User::$ROLE_ADMIN)
                    <th><a class="btn btn-primary pull-right" href="{{ route('admin.ecg_library.create') }}">追加</a></th>
                    @endif
                </tr>
            </thead>
            ...
            <tbody id="<?php echo isset($_GET['reorder']) ? "stetho_sound_list__tbody" : "" ?>">
                ...
                <tr>
                    ...
                    <td class="text-break"><a
                            href="{{ route('admin.ecg_library.show', $stetho_sound->id) }}">{{$stetho_sound->title_en}}</a>
                    </td>
                    <td>-</td>
                    <td>-</td>
                    ...
                    @if(Auth::user()->role == App\User::$ROLE_ADMIN)
                    <td class="text-right">
                        <a class="btn btn-success " href="{{ url('admin/ecg_library/'.$stetho_sound->id.'/edit?page='.$stetho_sounds->currentPage()) }}">編集</a>
                        <form action="{{ route('admin.ecg_library.destroy', $stetho_sound->id) }}" method="POST" style="display: inline;"
        ...
        <div class="col-sm-offset-10 col-sm-2">
            <a class="btn btn-primary pull-right" href="{{ route('admin.ecg_library.create') }}" style="position: relative;">追加</a>
        </div>
        <div class="col-sm-offset-4 col-sm-8" style="margin-top:80px;">
            <a class="btn btn-primary" href="/admin/ecg_library<?php echo isset($_GET['reorder']) ? "" : "?reorder" ?>">
