...
<div class="page-header">
  <h1>Teacher administrator correction</h1>
</div>
...
<div class="row">
  <div class="col-md-12">
    ...
    <form action="{{ route('admin.users.teachers.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal">
     ...
     <div class="form-group @if($errors->has('enabled')) has-error @endif">
        ...
        <div class="col-sm-10">
          ...
          @if(count($exam_groups) > 0)
            @foreach($exam_groups as $university)
              <option id="univ_add__opt" value="{{$university->id}}" 
                @if($user->exam_group)
                  @if($university->id==$user->exam_group->id) selected @endif
                @endif
              >
                {{$university->name}}
              </option>
      ...
      <div class="clearfix">
        ...
        <a class="btn btn-default pull-right" href="{{ route('admin.users.teachers.index') }}" style="margin-right: .5em;">キャンセル</a>
      </div>
   </form>
 </div>
</div>
