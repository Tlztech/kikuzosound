...
<!--*********************************************************-->
<?php $i = 0?>
@foreach($bookmarks as $stetho_sound)
<!-- .sound_box -->
...
<li class="rec_bookmark_sound_box sound_box @if(empty($stetho_sound->sound_path) || $stetho_sound->lib_type==1) aus-sound_box @endif"
    ...
    <div class="sound_box_inner">
        ...
        <div class="audio">
            <div class="audiojsZ audiojsZ_{{$stetho_sound->id}}">
                <?php
                    $homesrc = $stetho_sound->sound_path.'?v='.env('ASSET_VER');
                ...
                <audio id="ssid_{{ $stetho_sound->id }}" data-id="{{ $stetho_sound->id }}" data-lib_type="bookmarks" {{$muted}} preload="auto"
                    src="<?= $homesrc ?>"></audio>
        ...
        <div class="sound_conts accordion_conts">
            ...
            ?>
                ...
                <div class="vid_open" style="padding-top:100px; display:none" oncontextmenu="return false;">
                    ...
                    <video class="{{$libClassName}}" playsinline loop muted width="530" height="240"
                        id="bookmarks_stetho_sound_video[{{ $stetho_sound->id }}]" controlslist="nodownload"
                        src="{{ $homesrc }}">
                </div>
                @endif
                @else
                ...
                <div id="recommended_bookmarks_cont-{{$sound->id}}" class="recommended_ipax_cont">
                ...
                <div class="img_slide home-page">
                    <div class="img_slide_inner">
                        <ul class="bxslider_{{$sound->id}}">
                            <li>
                                <img src="{{ $stetho_sound->image_path.'?v='.session('version')  }}"
            ...
            <div class="clearfix fav_wrapper">
                <button class="FavButton" data-confirm="{{trans('home.do_process')}}"
                    data-id="{{ $stetho_sound->id }}" data-favo="{{ $stetho_sound->favo }}"
                    data-aid="{{ $params['aid'] }}">@lang('home.remove_fave_btn')</button>
            </div>
        </div>
        <!-- /.sound_conts -->
    </div>
...
@if($auth == 1 && count($bookmarks)>1)
    <div class="home-sort-wrapper" style="">
        <a class="home-sort-btn"
            href="/mypage<?php echo isset($_GET['reorder']) ? "" : "?reorder" ?>">     
