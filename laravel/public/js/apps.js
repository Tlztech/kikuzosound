...
jQuery(($) => {
	$(document).ready(()=>{
		...
		var parent_iframe = window.parent.document.getElementById(id);
		document.body.style.zoom = parent_iframe?parent_iframe.dataset.size:1;
	});
})
...
async function doLearn(case_id,USER_TOKEN,lang,sound) {
	function getContentNormal(){
		const bodyImage = sound.body_image.replace(/^.*[\\\/]/, '')
		const configuration = checkNullConfig(JSON.parse(sound.configuration))
		...
		const settings = {
			...
			points: {
				aptm: {
					A: {
						audito_name: soundPath.a_sound_path?soundPath.a_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: (configuration.a.x)+"px,"+configuration.a.y+"px,"+configuration.a.r,
						volume_control: 1
					},
					P: {
						audito_name: soundPath.p_sound_path?soundPath.p_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: configuration.p.x+"px,"+configuration.p.y+"px,"+configuration.p.r,
						volume_control: 1
					},
					T: {
						audito_name: soundPath.t_sound_path?soundPath.t_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: configuration.t.x+"px,"+configuration.t.y+"px,"+configuration.t.r,
						volume_control: 1
					},
					M: {
						audito_name: soundPath.m_sound_path?soundPath.m_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
				...
				heart: {
					H1: {
						audito_name: soundPath.h1_sound_path?soundPath.h1_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: configuration.m.x+"px,"+configuration.m.y+"px,"+configuration.m.r,
						volume_control: 1
					},
					H2: {
						audito_name: soundPath.h2_sound_path?soundPath.h2_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: configuration.m.x+"px,"+configuration.m.y+"px,"+configuration.m.r,
						volume_control: 1
					},
					H3: {
						audito_name: soundPath.h3_sound_path?soundPath.h3_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: configuration.m.x+"px,"+configuration.m.y+"px,"+configuration.m.r,
						volume_control: 1
					},
					H4: {
						audito_name: soundPath.h4_sound_path?soundPath.h4_sound_path.replace(/^.*[\\\/]/, ''):"no_sound.mp3",
						position: configuration.m.x+"px,"+configuration.m.y+"px,"+configuration.m.r,
						volume_control: 1
					}
				},
				...
				"result":[
					{
						...
						"description_image":"description.jpg",
						"settings": settings,
...
//	stopAllSound();
	...
	$('.bodymap_audio_wrapper').empty();;
        ...
        setTimeout(function(){
        	if (window.matchMedia("(orientation: portrait)").matches) {
				$('#stethoscope').css({left:"0px", top: "20px"});
			}
			if (window.matchMedia("(orientation: landscape)").matches) {
				$('#stethoscope').css({left:"0px", top: "20px"});
			}
...
function getAnimationZoomPoints(position){
	var zoom = $("body").css("zoom");
	var x = position.left;
	var y = position.top;
	var factor = (1 / zoom) - 1
	//set adjustment for zoom functions
